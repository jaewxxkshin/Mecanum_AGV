...
void des_psi_Callback(const std_msgs::Float32::ConstPtr& val);
// ----------------------------------------------------------------------------
...
{   
	...
	ros::Subscriber des_psi_sub=nh.subscribe("/des_psi_pub",100, des_psi_Callback);
...
void yaw_ctrl()
{
	...
	cur_psi = rot.z;
	err_psi = des_psi- cur_psi;
	//std::cout << "atan2 : " << atan2((wp_r_y[1]-wp_r_y[0]),(wp_r_x[1]-wp_r_x[0])) <<  std::endl; 
	//std::cout << "rot.z + M_PI/2 : " << rot.z+M_PI/2 << std::endl;
	std::cout << "err psi : " << err_psi << "\n" << "current_psi : " << cur_psi << std::endl;
...
{
	des_psi = val->data;
	//std::cout << "des_psi : " << des_psi <<std::endl;
}